...
- Single field validation via `field.validate()`, here is an example on how to use in the From: 
  ```
  onFieldChange = {
      val textField = editor.fields.text(it.fieldId) ?: return@Form
      val validity = textField.validate()
      val status = validity.status.lowercase // can be "valid" or "invalid"
      val field = validity.component
      val pageId = validity.pageId
      val fieldPositionId = validity.fieldPositionId
      val rowValidites = validity.rowValidites // nullable if its not a table or a collection field
      val messages = validity.messages // deprecated and should not be used
      if (validity is Valid) {
          val validStatus = validity.status.lowercase // "valid"
      }
  }
  ```
...
- External Navigation Pt.3 — cell navigation and focus highlighting: `goto` supports `columnId` for row-form cell targeting and `focus: true` for auto-focus with highlighted border styles ([documentation](https://docs.joyfill.io/kotlin/guides/navigation))
