...
// Add
documentEditor.decorators.add(path, decorator)

// Read
val decorators = documentEditors.decorators.list(path)

// Update (matched by action)
decorator.label = "Open Review"
documentEditor.decorators.update(path, action = "open_review", decorator = decorator)

// Remove
documentEditor.decorators.remove(path, action = "open_review")
```
...
- Fixed crash when duplicate columnId or fieldId values were used
