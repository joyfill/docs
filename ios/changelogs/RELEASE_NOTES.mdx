...
- **Scoped validation** — the validate API now supports validating a specific path via `validate(path:)` (e.g. a page or a single field), not only the full form. -
- **Form footer** — added a `formFooter` modifier on the SDK so host apps can inject a footer across form screens.  ```swift Form(documentEditor: documentEditor)     .formFooter {         MyFooterView()     } ```  **Full Changelog**: https://github.com/joyfill/components-swift/compare/3.0.0-rc13...3.0.0-rc14
