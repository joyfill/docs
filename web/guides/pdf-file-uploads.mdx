...
You will be notified via the [`onUploadAsync`](/web/guides/image-upload-handling#onuploadasync) SDK handler when a user uploads a file. You will need to convert that PDF file into images using the conversion functionality from Step 1: PDF Conversion.

The `onUploadAsync` should return a resolved promise of url objects inside an array. Learn more about handling uploads and returning properly formatted data in our [PDF, Files, and Image Uploads Guide](/web/guides/image-upload-handling).
