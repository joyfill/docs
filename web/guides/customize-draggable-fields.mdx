...
JoyDoc(document.getElementById('joyfill-target'), {
  mode: 'edit',
  fieldOptions: fieldOptions
})
```
...
You can also pass custom icons to field options. The custom icons can either be an image url or svg. The `iconType` property must be specified as `url` in case of an imageUrl whereas it should be specified as `custom` in case of a custom svg.
