...
This release introduces **Column-Level Formulas** for table text columns, plus formula syntax improvements and a mobile-view indicator fix.
...
- **Column-Level Formulas:** A table text column's default value can now be an Excel-style formula, e.g. `=Qty*Price`. Every blank cell under that column shows the calculated result, and shows the formula itself when you click into it. Nothing is written back to the row: leaving a cell alone saves nothing, typing over it saves your text, and clearing it brings the formula back.
  * Works in the grid and the input group, collections (including nested child tables), the row-edit modal, and individually mapped or dragged table cells.
  * Required validation on a text column checks the calculated value, so a column filled in by a formula no longer gets flagged as incomplete. New rows no longer copy the formula in as literal text, so it keeps calculating for every row you add.
...
- **Required and Hidden Indicators in Mobile View:** The required red asterisk, the hidden yellow eye icon, and hidden nested collection tables were disappearing in the mobile view whenever field syncing was off. The mobile canvas is locked to read-only so a drag there can't rewrite the desktop layout, and these indicators were incorrectly tied to that same read-only state. They now follow the actual authoring mode, so mobile shows the same required and hidden state as desktop while staying non-interactive.
...
- **Cell-Level Formulas:** Cell-level formulas are now case-insensitive and no longer require A1-style operands. `=sum(a,d)` and `=a+d` now work the same as `=SUM(A1,D1)`.
...
This release introduces **Auto-Shrink** and **Auto-Height** for text fields, so long values can fit their configured bounds or grow to show the full content.

🚀 **New Features**

<span style={{display: 'inline-block', padding: '4px 12px', border: '1.5px solid #10b981', borderRadius: '6px', color: '#10b981', fontWeight: '600', fontSize: '13px', marginBottom: '12px'}}>ADDED</span>

- **Auto-Shrink for Text Fields:** Text and textarea fields can now automatically reduce their font size to help long values fit within the configured field bounds. Configure `autoShrink`, `maxLines`, and `maxLength`. Text fields also now correctly wrap their values when `textOverflow` is set to `wrap`.

- **Auto-Height for Text Fields:** Text and textarea fields can now automatically grow in height when their value exceeds the available space. This is supported in the builder and PDF-form pages. On fillable PDF pages, exported fields retain the same auto-shrunk font and grow to show the full value. Auto-height fields grow only when needed and retain their configured width and position.

🛠️ **Enhancements**

<span style={{display: 'inline-block', padding: '4px 12px', border: '1.5px solid #f97316', borderRadius: '6px', color: '#f97316', fontWeight: '600', fontSize: '13px', marginBottom: '12px'}}>CHANGED</span>

- **JoyDoc JSON Schema:** Updated the JoyDoc JSON schema to support `autoShrink`, `autoHeight`, and their associated field-position settings.
...
This release introduces **Conditional Required Logic** and **Cell Visibility Logic**, plus an explicit `multi` property for multiple choice fields.

🚀 **New Features**

<span style={{display: 'inline-block', padding: '4px 12px', border: '1.5px solid #10b981', borderRadius: '6px', color: '#10b981', fontWeight: '600', fontSize: '13px', marginBottom: '12px'}}>ADDED</span>

- **Conditional Required Logic:** Required validation can now be driven by conditions instead of a fixed flag. Rules can be configured on fields, table or collection columns, individual cells (evaluated per row), and nested collection child tables. Use `enforce` to make something required when conditions are met, or `unenforce` to lift a requirement. Cell-level rules can react to sibling columns in the same row.

- **Cell Visibility Logic:** Individual cells within a table or collection column can now be shown or hidden per row, based on sibling columns or fields elsewhere in the document. Column headers stay in place, hidden cells keep their stored values, and hidden cells are exempt from required validation.

🛠️ **Enhancements**

<span style={{display: 'inline-block', padding: '4px 12px', border: '1.5px solid #f97316', borderRadius: '6px', color: '#f97316', fontWeight: '600', fontSize: '13px', marginBottom: '12px'}}>CHANGED</span>

- **Multiple Choice `multi` Property:** Multiple choice fields now use an explicit `multi` property to control whether more than one option can be selected. Set `multi: true` to allow multiple selections. Without it, the field behaves as single choice.
