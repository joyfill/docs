...
## Compromised @joyfill npm beta releases
...
```powershell
Get-ChildItem -Path .\* -Recurse -Include package-lock.json,yarn.lock,pnpm-lock.yaml |
...
**If those come back empty, run one more pass.** The searches above require `joyfill` and `2773` to appear on the same line, which depends on the `resolved` URL your lockfile records. Offline installs and internal registry mirrors can rewrite or omit that URL, so an affected entry can be present without matching. This search is noisier but cannot miss:

```bash
grep -rn --include=package-lock.json --include=npm-shrinkwrap.json --include=yarn.lock \
  --include=pnpm-lock.yaml --include=package.json '2773' . 2>/dev/null
```

If you use Bun, `bun.lockb` is a binary file and neither search can read it — run `bun pm ls | grep joyfill` instead.
...
You are not affected by this advisory. Two precautions still apply to you, and neither is optional — both close the path by which a later install could still pull an affected build.
...
The affected versions execute code at import time, so an install alone is enough to have triggered the implant. Continue to [If you installed or imported an affected version](#if-you-installed-or-imported-an-affected-version).
...
We recognize how much work that is. We would rather flag it than leave anyone to discover it downstream.
