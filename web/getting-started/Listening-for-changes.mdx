...
- `onFileUpload` - File upload events
...
```jsx
onFileUpload(params, files) => Promise<Array<ImageObject>>
...
function MyForm() {
  ...
  return (
    <JoyDoc
      doc={document}
      onChange={handleChange}
      onFileUploadAsync={handleFileUploadAsync}
    />
  );
}
...
```jsx
{
  ...
  "url": "<https://s3.amazonaws.com/docspace.dev.documents/68a47e5d32dddce3ee2c31a5/documents/my-document/68ef8fb38f7d1d3e8348f894-1760530355509.jpg>",
...
const handleChange = (changelogs, updatedDoc) => {
  ...
  const themeChanges = changelogs.filter(c =>
    c.target === 'style.update' && c.change.theme
  );

  if (themeChanges.length > 0) {
    console.log('Theme changed to:', themeChanges[0].change.theme);
  }
...
### 3. Upload Events Not Working

**Problem:** File upload events are not being triggered.

**Solution:** Ensure you're using the correct event handler and that your upload function returns the expected format:

```jsx
const handleUploadAsync = async (params, files) => {
